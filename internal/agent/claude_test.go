...
func TestClaudeArgsCarriesAJSONSchema(t *testing.T) {
	// The schema stops being a suggestion in the prompt and becomes a contract
	// the CLI enforces: it is handed to the model as a forced tool call, so a
	// reply that ignores the shape cannot be produced rather than being caught
	// afterwards by the parser.
	schema := `{"type":"object","properties":{"tasks":{"type":"array"}},"required":["tasks"]}`
	args := ClaudeArgs(ClaudeOpts{Prompt: "go", JSONSchema: schema})
	...
	if found != schema {
		t.Errorf("--json-schema = %q, want the schema verbatim", found)
	}
...
func TestClaudeArgsOmitsTheFlagWithoutASchema(t *testing.T) {
	for _, a := range ClaudeArgs(ClaudeOpts{Prompt: "go"}) {
		if a == "--json-schema" {
			t.Fatal("no schema was asked for, so the flag must not appear")
		}
	}
}

func TestParseClaudeLineReadsStructuredOutput(t *testing.T) {
	// With a schema the answer stops being prose in a text block and arrives on
	// the result event as a validated object. Reading only the text block would
	// leave the final message empty — which is exactly how this looked before
	// it was understood.
	line := []byte(`{"type":"result","subtype":"success","is_error":false,` +
		`"session_id":"s","total_cost_usd":0.1,"usage":{"input_tokens":10,"output_tokens":5},` +
		`"result":"{\"tasks\":[]}","structured_output":{"tasks":[]}}`)
	ev, err := ParseClaudeLine(line)
	if err != nil {
		t.Fatalf("ParseClaudeLine: %v", err)
	}
	if ev.Kind != EventResult {
		t.Fatalf("kind = %v, want a result", ev.Kind)
	}
	if ev.Structured != `{"tasks":[]}` {
		t.Errorf("Structured = %q, want the validated object", ev.Structured)
	}
}

func TestStructuredOutputBecomesTheFinalMessage(t *testing.T) {
	// The whole point: what the parser on the other side reads is the object
	// the CLI validated, not whatever prose the model wrote alongside it.
	stream := `{"type":"system","subtype":"init","session_id":"s"}
{"type":"assistant","message":{"content":[{"type":"text","text":"Here is my analysis, at length..."}]},"session_id":"s"}
{"type":"result","subtype":"success","is_error":false,"session_id":"s","result":"{\"tasks\":[]}","structured_output":{"tasks":[]}}
`
	// The runner's own folding, which is what turns a stream into a Result.
	var res Result
	...
	if res.FinalText != `{"tasks":[]}` {
		t.Errorf("FinalText = %q, want the structured object rather than the prose", res.FinalText)
	}
}
